...
- **v1.9** — （開發中）利潤比對改版：移除手動輸入卡片，價格欄位直接點擊編輯
- **v1.8** — （開發中）精簡：移除未使用的價格表與截圖上傳頁，首頁直接導向利潤比對
- **v1.7** — （開發中）最佳單項利潤 + 剩餘配額顯示（含多次 UI 微調）
- **v1.6** — （開發中）囤貨追蹤：持有區自動偵測、利潤門檻建議、囤貨管理
- **v1.5** — ✅ 目前版本 — F2 OCR 文字優先、持有區過濾、好友列表雜訊過濾
