...
1. **價格 x 範圍：** `block['center_x'] < img_width * 0.70`（避開右邊百分比欄）
2. **單調遞減過濾：** 好友列表必為「販售價由高到低」，遇到反常變高的就視為雜訊跳過
3. **排除含 `%` / `.` 的文字：** 價格是純整數，百分比不是

**Why:** v2.0 實測時 OCR 把 `▲51.1%` 讀成 `5110`，只靠 x 範圍仍漏，必須兩道一起。
v6.0 界石鎖貨組跌到 430、漲幅爆成 `▲1079.1%`（4 位數），x 界線 0.75 擋不住百分比欄
（在 0.72），且它 y 座標比真價格更靠近名字而勝出，故收窄到 0.70 並加第 3 道。
