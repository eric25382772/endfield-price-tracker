...
**一條龍做到底**：改版號 → 更新說明過目 → push → 本機編 setup.exe → 上傳 release。不要 push 完就丟回給使用者。

**唯一要停下來問的是更新說明的文字**（步驟 1.5）——那是使用者唯一看得到的產出，也是唯一改一個字就要「重編兩個資產 + 重傳 + 使用者自己再更新一次」的東西。其他步驟一律自己做完。
...
1. [CHANGELOG.md](../../../CHANGELOG.md) — 加一行（技術細節，開發用）
   ...
   （網頁更新紀錄彈窗的內容。完整寫法規則以該檔 docstring 為準，重點：
   類別只用 新增／改善／修正，遊戲改版數值用 調整；一條一句話、從使用者角度寫，
   不寫原因、實作、檔名；修正寫「原本會發生什麼問題」、句尾統一「…的問題」，
   句首不要再寫一次「修正」；**同一個症狀的多項修正併成一條**；
   **只寫使用者看得到的現象，不寫壞掉的機制**）
...
## 1.5 把更新說明貼給使用者過目（**編譯前唯一的停等點**）

改完版號後、`git push` 之前，把 `release_notes.py` 這一版的條目**原樣列在主訊息裡**讓使用者看過再往下走：
...
**Why 一定要在這裡停：** 這幾行字會被封進 `update_*.zip` 與 `setup.exe`（兩者都含 `release_notes.py`），
release 的 `--notes` 也要同一份。發出去之後才發現要改一個字，代價是**重編兩個資產 + `--clobber` 重傳 +
使用者自己再更新一次**——v5.1.5 就這樣重來一遍。文字是唯一使用者直接看到的產出，事前 10 秒確認勝過事後全部重跑。

不要用 AskUserQuestion 面板（繁體字會顯示成亂碼），直接寫在主訊息裡。
